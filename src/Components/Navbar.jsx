...
const Navbar = () => {
  ...
  const navItems = [
    { name: "Overview", href: "#home", icon: <Book size={16} /> },
          ...
          <div className="hidden md:flex items-center space-x-6">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="flex items-center gap-2 text-sm font-semibold text-gray-300 hover:bg-gray-800/80 px-3 py-2 rounded-md transition-colors"
              >
                {item.icon}
                {item.name}
              </a>
            ...
            >
              {navItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="flex items-center gap-3 px-4 py-2 text-gray-300 hover:bg-gray-700 rounded-md"
